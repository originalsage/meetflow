...
- **Method**: `POST`
- **Auth**: 需要
- **Description**: 创建新的会议室预约。系统会进行以下验证：
  - 会议室是否可预约
  - 会议室容量是否足够
  - 该会议室在该时间段是否已被预约
  - **同一用户在同一时间段不能预约多个会议室**（时间段重叠检查）
- **Request Body**:
```json
{
  "meetingRoomId": 1,
...
**业务规则**:
- 同一用户在同一时间段（包括部分重叠）不能预约多个会议室
- 例如：如果用户已在9:00-11:00预约了会议室A，则不能再预约9:00-12:00或10:00-12:00的会议室B
- 时间段重叠判断：`startTime < endTime AND endTime > startTime`
...
- **Method**: `PUT`
- **Auth**: 需要（管理员或超级管理员）
- **Description**: 审批预约，可以批准或驳回。审批通过时会再次验证：
  - 会议室是否可预约
  - 会议室容量是否足够
  - 该会议室在该时间段是否已被其他预约占用
  - **该用户在该时间段是否已有其他预约**（时间段重叠检查）
- **Path Parameters**:
  - `id`: 预约ID（必填）
...
4. **时间段重叠限制**: 
   - 同一用户在同一时间段（包括部分重叠）不能预约多个会议室
   - 例如：如果用户已在9:00-11:00预约了会议室A，则不能再预约9:00-12:00或10:00-12:00的会议室B
   - 时间段重叠判断规则：`startTime < endTime AND endTime > startTime`
   - 此限制在创建预约和审批预约时都会进行检查
5. **文件上传**: 上传会议室照片时，文件大小建议不超过5MB，支持常见图片格式（jpg、png、gif等）
6. **分页参数**: 分页查询接口中，`page` 从1开始，`pageSize` 默认12
7. **权限验证**: 所有需要权限的接口都会验证Token，如果Token无效或权限不足会返回401或403错误
